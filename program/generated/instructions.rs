...
pub enum ConcertTicketNftInstruction {
/// Accounts:
...
pub struct MintArgs {
	...
	pub currency: String,
	pub limited_edition: String,
	pub additional_benefits: String,
	pub rating: String,
	pub schedule: String,
...
impl ConcertTicketNftInstruction {
    pub fn unpack(input: &[u8]) -> Result<Self, ProgramError> {
        let (&variant, rest) = input.split_first().ok_or(ConcertTicketNftError::InvalidInstruction)?;

        Ok(match variant {
			0 => Self::Mint(MintArgs::try_from_slice(rest).unwrap()),
			1 => Self::Transfer,
			2 => Self::Burn,
			_ => return Err(ConcertTicketNftError::InvalidInstruction.into())
        })
    }
}