...
use crate::generated::errors::ConcertTicketNftError;
use crate::generated::instructions::ConcertTicketNftInstruction;
...
impl Processor {
    pub fn process(
        ...
        let instruction = ConcertTicketNftInstruction::unpack(data)?;

        match instruction {
			ConcertTicketNftInstruction::Mint(args) => {
				...
				Self::process_mint(
					...
					args.ticket_type,
					args.currency,
					args.limited_edition,
					args.additional_benefits,
					args.rating,
					args.schedule,
					args.organizer,
					args.description,
			...
			ConcertTicketNftInstruction::Transfer => {
				msg!("Instruction: Transfer");
				Self::process_transfer(program_id, accounts)
			}
			ConcertTicketNftInstruction::Burn => {
...
/// - description: [String] type
	pub fn process_mint(
		...
		if fee_payer_info.is_signer != true {
			return Err(ConcertTicketNftError::InvalidSignerPermission.into());
		}
		...
		if *ticket_info.key != ticket_pubkey {
			return Err(ConcertTicketNftError::NotExpectedAddress.into());
		}
		...
		if *fee_payer_info.owner != Pubkey::from_str("11111111111111111111111111111111").unwrap() {
			return Err(ConcertTicketNftError::WrongAccountOwner.into());
		}
		...
		if mint_info.data_len() != spl_token::state::Mint::LEN {
			return Err(ConcertTicketNftError::InvalidAccountLen.into());
		}
