...
mod mint {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("concert_ticket__nft", program_id)
			.start();
		let rpc_client = test_validator.get_rpc_client();

		let ticket_type: String = Default::default();
		let currency: String = Default::default();
		let limited_edition: String = Default::default();
		let additional_benefits: String = Default::default();
		let rating: String = Default::default();
		let schedule: String = Default::default();
		let organizer: String = Default::default();
		...
		let (ticket_pubkey, _) = Pubkey::find_program_address(
			&[b"ticket", mint_info.pubkey().as_ref()],
			&program_id,
		);
		let accounts = vec![
		...
		let data = ConcertTicketNftInstruction::Mint(crate::generated::instructions::MintArgs{
			ticket_type,
			currency,
			limited_edition,
			additional_benefits,
